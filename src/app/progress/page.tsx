    ...
    try {
      ...
      if (result.success) {
        const processedData = result.data?.map(p => ({
          ...p,
          completionRate: p.completionRate != null ? `${p.completionRate.toFixed(1)}%` : 'N/A',
          onTimeRate: p.onTimeRate != null ? `${p.onTimeRate.toFixed(1)}%` : 'N/A',
          averageDelay: p.averageDelay != null ? `${p.averageDelay.toFixed(1)} 天` : 'N/A',
        })) || [];
